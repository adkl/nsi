...
namespace NSI.DocumentGenerator.Implementations
{
    public class DocxGenerator : IDocxGenerator
    {
        ...
        {
            File.WriteAllText(Path.Combine(workingDir, fileName + ".html"), content);
            ...
            Object docxFilePath = Path.Combine(workingDir, fileName + ".docx");

            wordDoc = word.Documents.Open(ref htmlFilePath, ref oMissing,
                ref readOnly);
            object finalFileFormat = WdSaveFormat.wdFormatXMLDocument;
            wordDoc.SaveAs(ref docxFilePath, ref finalFileFormat, ref oMissing, ref oMissing, ref oMissing,
            ...
            // Read file bytes 
            byte[] data = File.ReadAllBytes(Path.Combine(workingDir, fileName + ".docx"));
        ...
        public byte[] GenerateDocxFromHtml(string content)
        {
            try
            {
                ...
                // Possible extensions
                List<string> list = new List<string> { ".docx", ".html" };
            ...
            {
                throw new NsiBaseException(DocumentMessages.DocxGeneratorFailed, e, Common.Enumerations.SeverityEnum.Error);
            }
        }
    }
}
